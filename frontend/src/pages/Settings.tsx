...
function SendGridSetupModal({ isOpen, onClose, integration, settings, userEmail, inboundParse }: SendGridSetupModalProps) {
  ...
  const [copiedSendgridHost, setCopiedSendgridHost] = useState(false);

  const getDnsHostPrefix = (fullDomain: string): string => {
    const trimmed = fullDomain.trim().toLowerCase();
    if (!trimmed || trimmed.includes('@')) return 'reply';
  ...
  const [inboundDomainInput, setInboundDomainInput] = useState(
    inboundParse?.inboundDomain || (senderEmail && senderEmail.includes('@') ? `reply.${senderEmail.split('@')[1]}` : '')
  );
  const [inboundDomainErr, setInboundDomainErr] = useState('');

  const verifyInboundWebhookMutation = useMutation({
    mutationFn: () => {
      const domain = inboundDomainInput.trim().toLowerCase();
      if (!domain) {
        throw new Error('Inbound reply domain is required.');
      }
      if (domain.includes('@')) {
        throw new Error('Inbound reply domain cannot be an email address. Enter a domain name such as reply.acme.com.');
      }
          ...
          {wizardStep === 3 && (
            <div className="space-y-5">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                ...
                {isStep3Done ? (
                  <span className="text-xs bg-emerald-100 text-emerald-800 font-semibold px-2.5 py-0.5 rounded-full border border-emerald-300">
                    Inbound Reply Status: ✓ Ready
                  </span>
                ) : (
                  <span className="text-xs bg-amber-100 text-amber-800 font-semibold px-2.5 py-0.5 rounded-full border border-amber-300">
                    Awaiting Inbound Verification ⏳
                  </span>
                )}
              </div>

              {/* 1. INBOUND REPLY DOMAIN FIELD */}
              <div className="space-y-1.5 p-3.5 bg-slate-50 border border-slate-200 rounded-lg">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-slate-900 block">
                    1. Inbound Reply Domain
                  </label>
                  <span className="text-[11px] font-semibold text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-200">
                    DNS Host Name: <code className="font-mono">{getDnsHostPrefix(inboundDomainInput)}</code>
                  </span>
                </div>
                <p className="text-[11px] text-slate-600 leading-tight">
                  Enter the domain that will receive customer replies. Example: <code>reply.jakhar.xyz</code> or <code>inbound.acme.com</code>
                </p>
                <input
                  type="text"
                  ...
                  onChange={(e) => {
                    ...
                    setInboundDomainInput(val);
                    if (val.includes('@')) {
                      setInboundDomainErr('Inbound reply domain cannot be an email address. Enter a domain name such as reply.jakhar.xyz.');
                    } else {
                      setInboundDomainErr('');
                    }
                  }}
                  placeholder="reply.jakhar.xyz"
                  className="w-full p-2 border border-slate-300 rounded-md text-xs font-mono focus:ring-2 focus:ring-blue-500 bg-white"
                />
                {inboundDomainErr && (
                  <p className="text-[11px] text-red-600 font-medium">{inboundDomainErr}</p>
                )}
              </div>

              {/* 2. DNS PROVIDER INSTRUCTIONS CARD */}
              <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <h5 className="text-xs font-bold text-slate-900">2. Add MX Record to your DNS Provider</h5>
                    <p className="text-[11px] text-slate-600">Log into Cloudflare, Namecheap, GoDaddy, or Route53 and add this record under your domain settings:</p>
                  </div>
                  <span className="text-[11px] text-slate-500 font-medium">DNS Provider Setup</span>
                </div>
                  ...
                  <table className="w-full text-left text-xs border border-slate-200 bg-white rounded-md">
                    <thead className="bg-slate-100 text-slate-700 font-semibold border-b border-slate-200 text-[11px]">
                      <tr>
                        ...
                        <th className="px-3 py-1.5">Exact Value to Enter</th>
                    ...
                    <tbody className="divide-y divide-slate-100 font-mono text-[11px]">
                      ...
                      <tr>
                        ...
                        <td className="px-3 py-2 font-bold text-slate-900">{getDnsHostPrefix(inboundDomainInput)}</td>
                        <td className="px-3 py-2 font-sans">
                          <button
                            type="button"
                            onClick={() => handleCopyText(getDnsHostPrefix(inboundDomainInput), setCopiedDnsHost)}
                      ...
                      <tr>
                        ...
                        <td className="px-3 py-2 font-sans">
                          <button
                            type="button"
                            onClick={() => handleCopyText('mx.sendgrid.net', setCopiedMxTarget)}
                            className="px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded text-[10px] font-medium border border-slate-300"
                          >
                ...
                <div className="p-2.5 bg-amber-50 border border-amber-200 rounded text-[11px] text-amber-900 space-y-1">
                  <p className="font-bold flex items-center">
                    <span className="mr-1">💡</span> Important DNS Rule:
                  </p>
                  <p className="leading-normal text-[10.5px]">
                    In Cloudflare, Namecheap, or GoDaddy, enter <strong>only the host prefix (<code className="font-mono">{getDnsHostPrefix(inboundDomainInput)}</code>)</strong> in the Host box. Do NOT enter the full domain (<code className="font-mono">{inboundDomainInput.trim() || 'reply.jakhar.xyz'}</code>), or it will create a broken double domain record.
                  </p>
                </div>
              </div>

              {/* 3. SENDGRID PARSE SETUP INSTRUCTIONS CARD */}
              <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <h5 className="text-xs font-bold text-slate-900">3. Configure SendGrid Inbound Parse</h5>
                    <p className="text-[11px] text-slate-600">Click below to open SendGrid Inbound Parse, click <strong>Add Host & URL</strong>, and enter these values:</p>
                  </div>
                  <a
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border border-slate-200 bg-white rounded-md">
                    <thead className="bg-slate-100 text-slate-700 font-semibold border-b border-slate-200 text-[11px]">
                      <tr>
                        <th className="px-3 py-1.5">SendGrid Modal Box</th>
                    ...
                    <tbody className="divide-y divide-slate-100 font-mono text-[11px]">
                      <tr>
                        ...
                        <td className="px-3 py-2 text-slate-900 font-bold">{inboundDomainInput.trim() || 'reply.jakhar.xyz'}</td>
                        <td className="px-3 py-2 font-sans">
                          <button
                            type="button"
                            onClick={() => handleCopyText(inboundDomainInput.trim() || 'reply.jakhar.xyz', setCopiedSendgridHost)}
                            className="px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded text-[10px] font-medium border border-slate-300"
                          >
                            {copiedSendgridHost ? 'Copied Domain!' : 'Copy Domain'}
                          </button>
                        </td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 text-slate-700 font-sans font-bold">URL</td>
                        <td className="px-3 py-2 text-slate-800 break-all select-all font-mono text-[10px] max-w-[280px]">
                          {webhookUrl}
                        </td>
                        <td className="px-3 py-2 font-sans">
                          <button
                            ...
                            className="px-2 py-0.5 bg-slate-800 hover:bg-slate-900 text-white rounded text-[10px] font-medium shrink-0"
                          >
                      ...
                      <tr>
                        ...
                        <td className="px-3 py-2 text-slate-500 font-sans text-[11px]" colSpan={2}>
                          Leave <strong>Unchecked</strong>
                        </td>
                      </tr>
                    </tbody>
                  </table>
              ...
              {/* VERIFICATION TRIGGER BUTTON & STATUS */}
              <div className="pt-2 space-y-3">
                <div className="flex items-center space-x-2">
                  <button
                    type="button"
                    onClick={() => verifyInboundWebhookMutation.mutate()}
                    disabled={verifyInboundWebhookMutation.isPending || !!inboundDomainErr}
                  ...
                  >
                    {verifyInboundWebhookMutation.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
                    {verifyInboundWebhookMutation.isPending ? 'Checking DNS MX & Webhook...' : 'Re-check DNS & Webhook'}
                  </button>
                  <button
                    type="button"
                ...
                {inboundOtpErr && (
                  <div className="p-3 bg-red-50 border border-red-200 rounded-md text-xs text-red-700 font-medium leading-relaxed space-y-1">
                    <p className="font-bold">❌ Verification Failed:</p>
                    <p>{inboundOtpErr}</p>
                    <p className="text-[11px] text-red-600 font-normal">
                      Note: If you just added the MX record to Cloudflare/Namecheap, DNS propagation can take 1 to 5 minutes. Wait a minute and click <strong>Re-check DNS & Webhook</strong> again.
                    </p>
                  </div>
                )}
                {inboundOtpMsg && (
              ...
              {/* Multi-Tenant Virtual Sub-Addressing Explanation Banner */}
              <div className="p-3.5 bg-blue-50/70 border border-blue-200 rounded-md space-y-2 text-xs text-blue-900">
                <div className="flex items-center justify-between font-bold text-blue-950">
                  <span>How Virtual Sub-Address Routing Works:</span>
                </div>
                <ul className="list-disc list-inside space-y-1 text-[11px] text-blue-800 leading-relaxed">
                  <li><strong>No Real Mailbox Needed:</strong> Customer replies flow directly into Jaktra AI Dispute Engine via sub-addressing.</li>
                  <li><strong>Dynamic Virtual Addresses:</strong> Jaktra generates unique virtual reply addresses (e.g. <code>r_f8QkL9Xn...@reply.jakhar.xyz</code>).</li>
                  <li><strong>One-Time Domain Setup:</strong> SMTP routes all replies using the MX record of <code>{inboundDomainInput.trim() || 'reply.jakhar.xyz'}</code>. Once verified, millions of reply addresses under this domain are valid automatically.</li>
                  <li><strong>Automated Webhook Resolution:</strong> Jaktra securely resolves the unique reply token received in the webhook to the corresponding tenant and invoice.</li>
                </ul>
              </div>
              ...
              <div className="pt-4 flex justify-between items-center border-t border-slate-100">
                ...
                <button
                  ...
                  onClick={() => {
                    if (isAllDone) {
                      ...
                      setErrorMsg('Please click Re-check DNS & Webhook above to confirm inbound reply setup.');
                    }
                  }}
                ...
                >
                  {isAllDone ? 'Setup Complete & Connected ✓' : 'Re-check DNS & Webhook To Finish'}
                </button>
              </div>
            </div>
          )}
